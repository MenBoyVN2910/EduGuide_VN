...
class Visitor(VisitorBase, table=True):
    ...
    visit_events: list["VisitEvent"] = Relationship(back_populates="visitor", cascade_delete=True)

class VisitorPublic(VisitorBase):
    id: uuid.UUID
    created_at: datetime | None = None

class VisitEventBase(SQLModel):
    path: str = Field(max_length=512)

class VisitEvent(VisitEventBase, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    visitor_id: uuid.UUID = Field(foreign_key="visitor.id", nullable=False, ondelete="CASCADE")
    timestamp: datetime | None = Field(
        default_factory=get_datetime_utc,
        sa_type=DateTime(timezone=True),  # type: ignore
    )
    visitor: Visitor | None = Relationship(back_populates="visit_events")

class VisitEventPublic(VisitEventBase):
    id: uuid.UUID
    visitor_id: uuid.UUID
    timestamp: datetime | None = None

class VisitEventsPublic(SQLModel):
    data: list[VisitEventPublic]
    count: int
...
class AnalyticsChartData(SQLModel):
    date: str
    visits: int
...
# ── Neo4j Graph Models ──────────────────────────────────────
class CypherQuery(SQLModel):
    query: str

class CypherResult(SQLModel):
...
class GraphRelationship(SQLModel):
    id: str | int
    source: str | int
    target: str | int
    type: str
