      ...
      >
        <form onSubmit={handleSubmit}>
          <div className="px-7 py-6 border-b border-border/50 flex justify-between items-center">
            <div>
              <h2 className="text-xl font-bold tracking-tight">{t("notes.addTitle")}</h2>
              <p className="text-xs text-muted-foreground mt-0.5">{t("notes.addSubtitle")}</p>
            </div>
            <button 
              type="button" 
              onClick={onClose} 
          ...
          <div className="px-7 py-6 flex flex-col gap-5">
            <div className="flex flex-col gap-2">
              <label htmlFor="note-title" className="text-[13px] font-semibold text-foreground/80 ml-1">{t("notes.titleLabel")}</label>
              <input
                ref={titleRef}
                id="note-title"
                type="text"
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="note-content" className="text-[13px] font-semibold text-foreground/80 ml-1">{t("notes.contentLabel")}</label>
              <textarea
                id="note-content"
                value={content}
          ...
          <div className="flex justify-end gap-3 px-7 py-5 border-t border-border/50 bg-muted/10">
            <Button type="button" variant="ghost" className="rounded-xl px-5" onClick={onClose}>{t("notes.cancel")}</Button>
            <Button type="submit" className="rounded-xl px-6 gap-2 font-medium shadow-sm bg-primary hover:bg-primary/90">
              <Plus size={16} /> {t("notes.save")}
            </Button>
          </div>
        </form>
      </motion.div>
        ...
        <AnimatePresence mode="wait">
          {notes.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-2xl font-bold mb-2">{t("notes.emptyTitle")}</h3>
            ...
            >
               <p className="text-muted-foreground">{t("notes.noMatch")} "<span className="text-foreground font-medium">{search}</span>"</p>
              <Button type="button" onClick={() => setSearch("")} className="text-primary text-sm font-medium mt-2 hover:underline">{t("notes.clearSearch")}</Button>
            </motion.div>
          ) : (
            <motion.div
              key="results"
